...
def user():
    sql = "SELECT * FROM query where email='"+session["email"]+"'"
    data=[]
    stmt = ibm_db.exec_immediate(conn, sql)
    dictionary = ibm_db.fetch_both(stmt)
    while dictionary != False:
...
def user_create():
    sql = "INSERT INTO query (email,name , query , data)values('"+session["email"]+"','"+request.form["title"]+"','"+request.form["query"]+"','Waiting List')"
    stmt = ibm_db.exec_immediate(conn, sql)
    return redirect("/user")
...
def query_update(id):
    sql = "UPDATE query set data='"+request.form["data"]+"' where id='"+id+"'"
    stmt = ibm_db.exec_immediate(conn, sql)
    return redirect("/admin/home")
